...
3. Add `CityGrid`, `CityGridVisualizer`, `CityResources`, `BuildManager`, `EconomyTicker`, and `CityBuilderBootstrap` to scene objects.
...
## Permanent grid visualisation

Add `CityGridVisualizer` to the same GameObject as `CityGrid` to draw a runtime grid permanently in the Game view. Leave `showGrid` enabled, set `lineColor` for visibility, and adjust `yOffset` slightly above the terrain to prevent z-fighting.

## Resource UI

Use `ResourceDisplay` for a reusable current-amount label by assigning a `CityResources` reference and selecting the `ResourceType` to display. Use `ResourceDisplayGroup` on a parent panel to push the same `CityResources` reference into child displays. Use `ResourceRateDisplay` to show per-minute production for any resource type, such as gold per minute from completed gold mines.
